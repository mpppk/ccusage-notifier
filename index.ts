...
export async function handleCalendar(usage: ClaudeUsage, opts: NotifierOptions): Promise<void> {
  ...
  if (opts.calendarApi) {
    ...
    console.log(`[calendar] Inserting ${events.length} event(s) via API to calendar: ${calendarId}`);
    for (const ev of events) {
      try {
        const result = (await insertGoogleCalendarEvent(token, calendarId, ev)) as { htmlLink?: string; id?: string };
        console.log(`  ✓ ${ev.summary} -> ${result.htmlLink ?? result.id ?? "created"}`);
      } catch (e) {
